...
class form_stationery(models.Model):
    ...
    origin = fields.Char('Source Document', copy=False,
        help="Reference of the document that generated this purchasing"
...
#   Untuk memanggil metode ORM langsung dari sebuah object
    ...
    def action_stationery_send(self):
        '''
        ...
        try:
            if self.env.context.get('send_stationery', False):
        ...
        for order in self:
            for inv in order.invoice_ids:
                if inv and inv.state not in ('cancel', 'draft'):
                    raise UserError(_("Unable to cancel this purchsing stationery. You must first cancel the related vendor bills."))
...
class product_lines(models.Model):
    ...
    product_id = fields.Many2one('product.template', string='Name Product', domain=[('purchase_ok', '=', True)], change_default=True)
    date_planned = fields.Datetime(string='Request Date', index=True)
